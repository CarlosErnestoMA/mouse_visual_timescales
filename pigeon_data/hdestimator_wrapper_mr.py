...
def hde(spiketimes, hde_path="/home/marcelraabe/projects/hdestimator", cli_args=dict()):

    if not hde_path in sys.path:
        sys.path.insert(0, hde_path)

    # there is an estimate.py in the hdestimator folder
    from estimate import hde_estimate_main as est_main
    ...
    with tempfile.NamedTemporaryFile(
        ...
        with h5py.File(spikefile.name, "w") as f:
            ...
            #df = pd.read_csv(analysis_dir + "/statistics.csv") # MR
